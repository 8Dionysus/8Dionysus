...
PACKAGE_SCRIPT_REL = Path("mcp/services/abyss-machine-mcp/scripts/abyss_machine_mcp_server.py")
...
def candidate_scripts(root: Path) -> list[Path]:
    ...
    direct = os.environ.get("ABYSS_MACHINE_MCP_SERVER")
    if direct:
        paths.append(Path(direct).expanduser())

    package_root = os.environ.get("ABYSS_MACHINE_MCP_ROOT")
    if package_root:
        paths.append(Path(package_root).expanduser() / "scripts" / "abyss_machine_mcp_server.py")
...
def main() -> int:
    ...
    print(
        "Could not locate abyss-machine-mcp server. Set AOA_ABYSS_STACK_ROOT, "
        "ABYSS_MACHINE_MCP_ROOT, or ABYSS_MACHINE_MCP_SERVER.\nSearched:\n" + searched,
        file=sys.stderr,
    )
    return 1
