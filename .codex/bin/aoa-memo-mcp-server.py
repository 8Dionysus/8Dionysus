...
PACKAGE_SCRIPT_REL = Path("mcp/services/aoa-memo-mcp/scripts/aoa_memo_mcp_server.py")
...
def candidate_scripts(root: Path) -> list[Path]:
    ...
    direct = os.environ.get("AOA_MEMO_MCP_SERVER")
    if direct:
        paths.append(Path(direct).expanduser())

    package_root = os.environ.get("AOA_MEMO_MCP_ROOT")
    if package_root:
        paths.append(Path(package_root).expanduser() / "scripts" / "aoa_memo_mcp_server.py")
...
def main() -> int:
    ...
    print(
        "Could not locate aoa-memo-mcp server. Set AOA_ABYSS_STACK_ROOT, "
        "AOA_MEMO_MCP_ROOT, or AOA_MEMO_MCP_SERVER.\nSearched:\n" + searched,
        file=sys.stderr,
    )
    return 1
