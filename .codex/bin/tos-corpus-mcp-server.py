...
PACKAGE_SCRIPT_REL = Path("mcp/services/tos-corpus-mcp/scripts/tos_corpus_mcp_server.py")
...
def candidate_scripts(root: Path) -> list[Path]:
    ...
    direct = os.environ.get("TOS_CORPUS_MCP_SERVER")
    if direct:
        paths.append(Path(direct).expanduser())

    package_root = os.environ.get("TOS_CORPUS_MCP_ROOT")
    if package_root:
        paths.append(Path(package_root).expanduser() / "scripts" / "tos_corpus_mcp_server.py")
...
def main() -> int:
    ...
    print(
        "Could not locate tos-corpus-mcp server. Set AOA_ABYSS_STACK_ROOT, "
        "TOS_CORPUS_MCP_ROOT, or TOS_CORPUS_MCP_SERVER.\nSearched:\n" + searched,
        file=sys.stderr,
    )
    return 1
