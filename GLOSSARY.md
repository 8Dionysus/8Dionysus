...
The goal is not to inflate terminology.
The goal is to keep boundaries, roles, and relationships legible.

## Core directions

### Agents of Abyss (AoA)
An operational federation of explicit layers used to build, route, validate, and run long-horizon agentic systems.
...
Its current public layers include reusable practice, bounded execution, portable proof, navigation, memory, agent roles, scenario composition, and derived knowledge substrate work, with `abyss-stack` beneath the system and `Tree-of-Sophia` as the knowledge architecture counterpart.

### Tree of Sophia (ToS)
A source-first living knowledge architecture for philosophy and world thought.

ToS is the knowledge world and long-horizon meaning architecture that AoA helps build, maintain, route around, validate, and extend.
It owns authored knowledge truth about texts, concepts, lineages, context layers, and interpretive architecture.
...
It does not own the authored meaning of the specialized AoA layers or ToS source nodes.

### ATM10-Agent
A local-first companion application surface shaped by the same broader direction as AoA and ToS.

It combines perception, memory, safe automation, voice, and an operator panel in a reproducible local stack.
It is adjacent to the public AoA layer federation rather than a replacement for it.
...
`Agents-of-Abyss` and `aoa-routing` are coordination-oriented surfaces with different jobs, and this profile repository is a lighter public entry surface.
...
Coordination, routing, memory, and derived surfaces should not quietly replace source repositories.
...
It is a higher-level scenario composition.
...
It is not the primary source of authored meaning.
It is the derived substrate built from authoritative sources.
...
A playbook may compose roles, skills, memory posture, evaluation posture, handoffs, fallback paths, and expected evidence.

Method lives primarily at the playbook layer.
A playbook is not a single skill.
...
It is one of the main design goals across AoA and the profile surface.
