...
`8Dionysus`, use the repo-local `aoa-workspace-diagnose` home skill. The
installed `aoa-shared-launchers` plugin now contains only the explicit
`aoa-growth-snapshot` and `aoa-seed-route-inspect` candidates while their
separate owner-first audits remain pending.
Keep statistical meaning exposed through `aoa_stats` with `aoa-stats`;
the access plane and launcher packaging cannot strengthen it.
