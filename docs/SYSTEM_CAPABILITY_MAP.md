...
| Growth refinery and lineage | carry a reviewed session route through checkpoint hints, reviewed donor harvest, `candidate_ref`, owner-local intake, owner landing, proof, bounded memo, and derived funnel visibility | `Agents-of-Abyss` for doctrine, `aoa-sdk` for carry-only hints, `aoa-skills` for reviewed `candidate_ref`, owner repos for intake and landed objects | `AOA-P-0025` when the full reviewed cycle is in play; `AOA-P-0026` when a candidate already exists and only the next owner move is in scope; `AOA-P-0021` for owner-first landing | `aoa-stats:generated/candidate_lineage_summary.min.json`, `aoa-evals` lineage and owner-fit bundles, `aoa-memo:docs/GROWTH_REFINERY_WRITEBACK.md` |
...
- `Dionysus` keeps personal portrait claims evidence-grounded, human-reviewed,
  and purpose-bounded
