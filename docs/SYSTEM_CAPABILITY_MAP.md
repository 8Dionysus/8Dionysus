...
| MCP and access plane | expose project-level workspace orientation, repo-local observability, seed-routing, memory-access, session-evidence access, bounded proof-access, decision-lane lookup, and host-machine-context MCP surfaces, and let launcher skills bind them by stable name, without turning MCP or launcher packaging into authority | `aoa-sdk` owns `aoa_workspace`; `aoa-stats` owns `aoa_stats`; `Dionysus` owns `dionysus`; `abyss-stack` owns runnable `aoa_memo`, `aoa_session_memory`, `aoa_evals`, `aoa_decisions`, and `abyss_machine` services; `.aoa` owns raw session evidence exposed by `aoa_session_memory`; `aoa-evals` owns proof meaning exposed by `aoa_evals`; `aoa-skills` owns decision-route meaning exposed through `aoa_decisions`; `abyss-machine` owns host truth exposed by `abyss_machine`; `aoa-skills` owns skill-to-MCP dependency wiring; `aoa-agents` owns subagent projection affinity; `8Dionysus` owns the shared-root `.codex` install surface, launcher packaging, and convergence wrappers | use whenever orientation, stats observability, seed routing, memory access, session evidence, bounded proof access, decision-lane lookup, host-machine context, or Codex launch discipline is the need; MCP stays support infrastructure, not sovereign workflow truth | `AGENTS.md`, `.agents/plugins/marketplace.json`, `.codex/plugins/aoa-shared-launchers/PLUGIN_NOTES.md`, `aoa-sdk:docs/codex-workspace-mcp.md`, `aoa-skills:docs/CODEX_SKILL_MCP_WIRING.md`, `aoa-skills:.agents/skills/aoa-decision/SKILL.md`, `aoa-agents:docs/CODEX_SUBAGENT_PROJECTION.md`, `Dionysus:docs/CODEX_MCP.md`, `abyss-stack:mcp/services/aoa-memo-mcp/README.md`, `abyss-stack:mcp/services/aoa-session-memory-mcp/README.md`, `abyss-stack:mcp/services/aoa-evals-mcp/README.md`, `abyss-stack:mcp/services/aoa-decisions-mcp/README.md`, `abyss-stack:mcp/services/abyss-machine-mcp/README.md`, `aoa-evals:docs/architecture/AOA_EVALS_MCP_CONTRACT.md`, `abyss-machine:/etc/abyss-machine/AGENTS.md` |
