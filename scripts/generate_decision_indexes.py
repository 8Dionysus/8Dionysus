#!/usr/bin/env python3
"""Generate docs/decisions lookup indexes from 8Dionysus decision metadata."""
...
DECISION_PREFIX = "8DION-D"
DECISION_RE = re.compile(r"^8DION-D-(\d{4})-[a-z0-9][a-z0-9-]*\.md$")
DECISION_ID_RE = re.compile(r"^8DION-D-\d{4}$")
...
def collect_decision_records(repo_root: Path = REPO_ROOT) -> tuple[list[DecisionRecord], list[tuple[str, str]]]:
    ...
    for path in sorted(decision_root.glob("*.md")):
        ...
        if not path.name.startswith(f"{DECISION_PREFIX}-"):
            issues.append((rel, "top-level decision markdown must use 8DION-D-####-slug.md"))

    for path in sorted(decision_root.glob(f"{DECISION_PREFIX}-*.md")):
        rel = path.relative_to(repo_root).as_posix()
        match = DECISION_RE.match(path.name)
        if not match:
            issues.append((rel, "filename must match 8DION-D-####-slug.md"))
            continue
        ...
        route_anchors = _metadata_value(metadata, "Route anchors", rel, issues)
        ...
        if decision_id and not DECISION_ID_RE.match(decision_id):
            issues.append((rel, "Decision ID must match 8DION-D-####"))
        ...
        if decision_id and original_date and title:
            records.append(
                DecisionRecord(
                    ...
                    route_anchors=_split_values(route_anchors),
    ...
    if numbers:
        ...
        if numbers != expected_numbers:
            issues.append((DECISIONS_DIR.as_posix(), "decision numbers must be contiguous from 8DION-D-0001"))
...
def _table(records: list[DecisionRecord], *, columns: tuple[str, ...]) -> str:
    ...
    for record in records:
        ...
        for column in columns:
            ...
            elif column == "Route anchors":
                values.append(", ".join(record.route_anchors))
...
def render_index_files(records: list[DecisionRecord]) -> dict[Path, str]:
    ...
    return {
        INDEX_DIR / "README.md": (
            ...
            "- [By route anchor](by-route-anchor.md)\n"
        ...
        INDEX_DIR / "by-route-anchor.md": _grouped_index(
            "Decisions By Route Anchor",
            _group_by(records, "route_anchors"),
        ),
